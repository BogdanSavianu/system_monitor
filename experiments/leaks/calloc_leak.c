...
int main(int argc, char **argv) {
    ...
    if (kb_per_step == 0) {
        fprintf(stderr, "kb_per_step must be > 0\n");
        return 1;
    }

    const size_t member_size = 1024;
    size_t nmemb = (size_t)kb_per_step;
    const char *scenario = "calloc_leak";
    const int label = 1;
