...
Options:
  ...
  --scenario-jobs <n>  Parallel scenarios per run batch
  --skip <list>        Comma-separated list: train,valid,test
  -h, --help           Show this help message

Examples:
  ./experiments/leaks/prepare_train_valid_test_datasets.sh
  ./experiments/leaks/prepare_train_valid_test_datasets.sh --runs 30 --steps 300 --jobs 4 --scenario-jobs 7
...
if [[ "$SCENARIO_JOBS" -lt 1 ]]; then
  echo "--scenario-jobs must be >= 1" >&2
  exit 1
fi
...
echo "runs=$RUNS_PER_SCENARIO steps=$STEPS jobs=$JOBS scenario_jobs=$SCENARIO_JOBS skip=${SKIP_LIST:-<none>}"
...
generate_one() {
  ...
  "$LEAKS_DIR/generate_dataset.sh" "$out_dir" "$RUNS_PER_SCENARIO" "$STEPS" "$JOBS" "$SCENARIO_JOBS"
}
