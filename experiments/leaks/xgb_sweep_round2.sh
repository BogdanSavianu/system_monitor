...
cd "$(dirname "$0")/../.."
out_dir="experiments/real_retrain_xgb_round2/sweep"
mkdir -p "$out_dir"
...
for cfg in "${configs[@]}"; do
  ...
  if cargo run --release --manifest-path experiments/ml-trainer/Cargo.toml -- \
    --manifest experiments/real_manifests/train.txt \
    --valid-manifest experiments/real_manifests/valid.txt \
...
cargo run --release --manifest-path experiments/ml-trainer/Cargo.toml -- \
  --manifest experiments/real_manifests/holdout.txt \
  --window 24 \
